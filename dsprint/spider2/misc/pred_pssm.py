...
def read_pssm(pssm_file):
	...
	for line in lines:
		...
		if len(split_line) < 1 or not split_line[0].isdigit(): continue
...
#	SS_1 = [SS_order[i] for i in numpy.argmax(pred_ss_1,1)]
	SS_1 = [SS_order[i] for i in numpy.asarray(numpy.argmax(pred_ss_1,1)).flatten()]
	...
	for ind, x in enumerate(aa):
		...
		fp.write(('%i\t%c\t%c\t%5.1f' + '\t%6.1f'*4 + '\t%.3f'*3 + '\n') % (ind+1, x, SS_1[ind], asa, phi[ind], psi[ind], theta[ind], tau[ind], pred_ss_1[ind,0], pred_ss_1[ind,1], pred_ss_1[ind,2]))
	fp.close()
