...
try:
    ...
    import sys
    if len(sys.argv) != 7:
        print('Usage: <script> <domain> <hmm_states_folder> <canonic_prot_folder> <jsd_folder> <is_legacy> <output_pik_file>')
        sys.exit(0)

    DOMAIN, HMM_STATES_FOLDER, CANONIC_PROT_FOLDER, JSD_FOLDER, LEGACY, OUTPUT_FILE = sys.argv[1:]
    LEGACY = LEGACY == 'True'
else:
    DOMAIN = snakemake.params.hmm
...
if __name__ == '__main__':
    ...
    with open(f'{HMM_STATES_FOLDER}/{DOMAIN}.0.pik', 'rb') as f:
